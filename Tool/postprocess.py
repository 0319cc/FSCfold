...
def constraint_matrix_batch(x):             # x是一个三维张量，形状是[batch_size, seq_length, 4]
    """
    ...
    base_c = x[:, :, 2]
    base_g = x[:, :, 3]
    batch = base_a.shape[0]         # 批次大小
    length = base_a.shape[1]        # 序列长度
    ...
    return au_ua + cg_gc + ug_gu                    # 返回一个形状为 [batch_size, seq_length, seq_length] 的约束矩阵，表示在 RNA 序列中哪些碱基可以配对。
...
def soft_sign(x):           # logistic函数实现的sign函数
    k = 1
...
def postprocess(u, x, lr_min, lr_max, num_itr, rho=0.0, with_l1=False,s=math.log(9.0)):
    ...
    :return:
    """
    # 将 RNA 序列 x 转换为约束矩阵 m
    m = constraint_matrix_batch(x).float()      # 返回一个形状为 [batch_size, seq_length, seq_length] 的约束矩阵，表示在 RNA 序列中哪些碱基可以配对。
    # u with threshold
    # equivalent to sigmoid(u) > 0.9
    # u = (u > math.log(9.0)).type(torch.FloatTensor) * u
    # 对实用性矩阵 u 应用 soft sign 函数。
    u = soft_sign(u - s) * u
